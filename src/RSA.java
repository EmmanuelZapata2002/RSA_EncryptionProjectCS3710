//Emmanuel Zapata
//Computing ID: ez4fp@virginia.edu
// CS 3710- Cyber-security
// Due 05/30/2023
...
public class RSA {
    ...
    public static RSAKey generateKeys (int bitLength) throws Exception {
        ...
        // From Notes: Choose 2 random large prime#'s; p and q such that p is not equal to q. Key Generation Step 1
        int certainty = 100;
        int numBits = bitLength;
        Random random = new Random();
        BigInteger p = new BigInteger(numBits,certainty, random);
        BigInteger q;
        // Below the while loop makes sure that the generation of q involves a different value than that of p.
        // Q and P cannot be equal.
        do{
            q = new BigInteger(numBits, certainty, random);
        }
        while(p.equals(q));
        // this is for the show-p-and-q test case in the write-up.
        if(showPandQ){
        ...
        BigInteger e;
        // while loop ensures that e is relatively prime to pq AKA gcd of pq and e is 1.
        do{
            e = new BigInteger(numBits, certainty, random);
        }
        // Below are the conditions for e that have to be met; continue until gcd is 1 and when e is less than n
        // and greater than 1
        while(!(e.compareTo(n) < 0 && e.compareTo(BigInteger.ONE) > 0 && e.gcd(pq).equals(BigInteger.ONE)));

        // Generation of d from Slides.
        ...
        keys.e = e;
        keys.n = n;
        keys.bitLength = bitLength;

        // Here we are destroying the records of p and q, which is the 4th key generation step.
        p = null;
        q = null;
    ...
    public static CipherText encrypt (RSAKey key, String plainText) throws Exception {
        ...
        // Allows for the creation of the blocks when encrypting.
        int size_sep = (key.bitLength / 8) - 1; // Size of each block in characters
        BigInteger e = key.e;
        BigInteger n = key.n;

        for (int i = 0; i < plainText.length(); i += size_sep) {
            int stop_Index = i + size_sep;
            if (stop_Index > plainText.length()){
                // This is done to avoid going out of bounds when creating blocks.
                stop_Index = plainText.length();
            }
            //extract portion of the plaintext in blocks.
            String block = plainText.substring(i, stop_Index);

            // Convert the block to a BigInteger
            BigInteger plaintext_Block = convertFromASCII(block);

            // Perform modular exponentiation using the public key (e) and n
            BigInteger encrypted_Block = plaintext_Block.modPow(e,n);

            // Add the encrypted block to the cipherText
            cipherText.encryptedBlocks.add(encrypted_Block);
        }

        cipherText.fileLength = plainText.length();
        cipherText.blockLength = size_sep;
    ...
    public static String decrypt (RSAKey key, CipherText cipherText) throws Exception {
        // your code here; a dummy return statement is put below to allow this to compile
        //Initialization of string for decrypted message.
        String decrypted_message = "";
        BigInteger d = key.d;
        BigInteger n = key.n;

        // size_sep is the block length used. Will be the same for encryption and decryption.
        int size_sep = cipherText.blockLength;


        for (int i = 0; i < cipherText.encryptedBlocks.size(); i++){
            // retrieves ith encrypted block.
            BigInteger encrypted_Extract = cipherText.encryptedBlocks.get(i);
            // performs modular exponentiation using the private key and n
            BigInteger decrypted_Extract = encrypted_Extract.modPow(d, n);
            // replaces ith block in the encrypted message with the decrypted block.
            cipherText.encryptedBlocks.set(i, decrypted_Extract);

            // After converting to ASCII string, add onto the decrypted message string.
            decrypted_message += convertToASCII(decrypted_Extract);
    ...
    public static RSAKey crack (RSAKey key) throws Exception {
        ...
        BigInteger e = key.e;
        BigInteger n = key.n;
        BigInteger smallest_prime = BigInteger.valueOf(3);
        BigInteger increment = BigInteger.ONE;

        for (BigInteger i = smallest_prime; i.compareTo(n) < 0; i = i.add(increment)) {
            // if the integer i is a factor of n (mod = 0), then the condition is true and p = to the integer i.
            if (n.mod(i).equals(BigInteger.ZERO)) {
                p = i;
                break;
            }
        }
        // Solving for q once p has been solved. These are the factors of n being solved here.
        q = n.divide(p);

        BigInteger pq = p.subtract(BigInteger.ONE).multiply(q.subtract(BigInteger.ONE));
        //Solving for d here as usual.
        BigInteger d = e.modInverse(pq);

        RSAKey key_Cracked = new RSAKey();
        key_Cracked.bitLength = key.bitLength;
        key_Cracked.n = n;
        key_Cracked.d = d;
        key_Cracked.e = e;
    ...
    public static CipherText sign (RSAKey key, String plainText) throws Exception {
        // your code here; a dummy return statement is put below to allow this to compile
        // Copying d over to e before passing into encrypt.
        String hashAlgorithm = "SHA-256";
        // Here I am copying d over to e before passing it into encrypt().
        BigInteger d = key.d;
        BigInteger e = key.e;
        key.d = e;
        key.e = d;
        //Message Digest allows for computation of hash values.
        MessageDigest digest = MessageDigest.getInstance(hashAlgorithm);
        // hash value of plain text received and then stores it into a byte.
        byte[] hash_bytes = digest.digest(plainText.getBytes());

        //Converting to hexadecimal string.
        String hashed_message_after_Convert = convertHash(hash_bytes);

        //Encryption performed to retrieve cipher text object.
        CipherText cipherText = encrypt(key,hashed_message_after_Convert);
    ...
    public static boolean checkSign (RSAKey key, String plainText, CipherText signature) throws Exception {
        ...
        //Same Process as in my sign method.
        MessageDigest digest = MessageDigest.getInstance(hashAlgorithm);
        byte[] hash_bytes = digest.digest(plainText.getBytes());

        String hashed_message_after_Convert = convertHash(hash_bytes);
        // Instead of encrypting you are decrypting it now.
        String decrypted_extract = decrypt(key, signature);

        //Testing whether hashed messages match up or not.
        boolean signatures_Match = decrypted_extract.equals(hashed_message_after_Convert);
